...
export async function GET(req: NextRequest) {
  ...
  const base = parseInt(searchParams.get("base") ?? "3");
...
`);
    ...
    // Valores reais por dia do mês atual
    const realPorDiaRows = await query<{ dia_num: string; total: string }>(`
      SELECT
        EXTRACT(DAY FROM data_pgto)::INT AS dia_num,
    ...
    // Média histórica por dia (todos os dias do mês)
    const mediaPorDiaRows = await query<{ dia_num: string; media_dia: string }>(`
      WITH historico AS (
        SELECT
          EXTRACT(DAY FROM data_pgto)::INT AS dia_num,
          DATE_TRUNC('month', data_pgto) AS mes,
    ...
    const diasMes = Math.round(Number(diasRow?.dias_mes ?? 30));
    const diasPassados = Math.round(Number(diasRow?.dias_passados ?? 1));

    // Montar array com todos os dias do mês
    const realMap: Record<number, number> = {};
    realPorDiaRows.forEach(r => { realMap[Number(r.dia_num)] = Number(r.total); });

    const mediaMap: Record<number, number> = {};
    mediaPorDiaRows.forEach(r => { mediaMap[Number(r.dia_num)] = Number(r.media_dia); });

    const graficoDias = Array.from({ length: diasMes }, (_, i) => {
      const dia = i + 1;
      return {
        dia,
        real: dia <= diasPassados ? (realMap[dia] ?? 0) : null,
        projetado: mediaMap[dia] ?? 0,
      };
    });
